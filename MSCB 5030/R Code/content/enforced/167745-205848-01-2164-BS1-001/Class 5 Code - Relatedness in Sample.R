...
cor_store = rep(NA, 100000)

for(i in 1:100000){                 ## Simulate 1000 correlations this way.
  X = rnorm(500, mean=5, sd = 2)
  Y = rnorm(500, mean=2, sd = 6)
  cor_store[i] = cor(X,Y)
}
...
plot(X,W, main = "Plot of W versus X")   ## Note that a distinct pattern is in the plot.

lines(lowess(W~X), lty = "dashed", col="red")   ## Draw an approximation to E[W | X]
